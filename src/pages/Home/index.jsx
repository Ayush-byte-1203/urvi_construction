...
const Home = () => {
  ...
  const blogsList = Object.values(blogsData).slice(0, 3);
  ...
  return (
    <div className="home-page">
          ...
          {cityPackages.length > 0 ? (
            <div className="grid-3">
              {cityPackages.map((pkg, idx) => (
                ...
                >
                  {pkg.popular && <span className={styles.popularTag}>Popular Selection</span>}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: 'auto' }}>
                    <Link to="/packages">
                      <Button variant={pkg.popular ? 'primary' : 'secondary'} fullWidth>
      ...
      {/* 12. Latest Knowledge Center Articles */}
      <section className={`section ${styles.featuredSection}`}>
        <div className="container">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: '2rem', marginBottom: '3rem', flexWrap: 'wrap' }}>
            <div>
              <span className="text-overline">Knowledge Base</span>
              <h2 className="display-sm" style={{ marginTop: '0.5rem' }}>Insights on Engineering & Tech</h2>
            </div>
            <Link to="/blog">
              <Button variant="secondary">All Articles <ArrowRight size={16} /></Button>
            </Link>
          </div>

          <div className="grid-2">
            {blogsList.map((blog, idx) => (
              <MotionWrapper key={blog.id} variant="slideUp" delay={idx * 0.15}>
                <GenericCard
                  image={blogImages[blog.id] || 'https://images.unsplash.com/photo-1518005020951-eccb494ad742?auto=format&fit=crop&w=600&q=80'}
                  badge={blog.category}
                  title={blog.title}
                  description={blog.excerpt}
                  meta={[
                    <span key="date">{blog.date}</span>,
                    <span key="read">{blog.readTime}</span>
                  ]}
                  ctaText="Read Article"
                  ctaLink={`/blog/${blog.id}`}
                />
              </MotionWrapper>
            ))}
          </div>
